...
// 루차장님이 COMMUNICATION_Ethernet_STEPRead 함수 수정하심.
  ...
  length = len;
    ...
    while (length)
    {
      ...
      {   gui_StartFlag = HEADER_DATA;
          ...
          gul_SumCount  = ((buf[1] * COMMUNICATION_INTEGER_LENGTH_RX) + COMMUNICATION_INTEGER_LENGTH_HEADER) * 2; //수신 DATA 총 갯수 정보 복사
          gui_Channel   = buf[2]; //장비 정보 복사
          //상태 표시 변수 초기화
          gul_DataCount = CLEAR;
          for(ui_i = 0; ui_i < COMMUNICATION_CHANNEL; ui_i++)gPointerCount[ui_i] = CLEAR;
      }
      gul_DataCount += length; //8BIT 단위의 Ethernet에서 수신된 BigEndian 자료수신 갯수
      ...
      if(gul_DataCount >= gul_SumCount)//Data 총 갯수와 실제로 Counting한 변수를 비교후 같거나
		...
		volatile S_COMMUNICATION_GSRAM_CPU2 *sp_GSRAM_CPU2, Uint16 *buf, Uint32 len)
{
	...
	ul_DataCountTotal = (Uint16)((Uint32)(len) / (Uint32)(sizeof(S_COMMUNICATION_RX_JUMP) * 2)); //8BIT 단위의 Ethernet에서 수신된 BigEndian 자료수신 갯수

	for(ui_i = 0; ui_i < ul_DataCountTotal; ui_i++)
	{	//4개의 채널에서 해당 되는 Channel을 확인하고 해당 SETP DATA에 자료 저장
	    ui_ChannelIndex = CLEAR;
		ul_DataCount  = buf[1 + PointerCount]; //수신 DATA 총 갯수 정보 복사
		...
		volatile S_COMMUNICATION_GSRAM_CPU2 *sp_GSRAM_CPU2, Uint16 *buf, Uint32 len)
{
	...
	ul_DataCountTotal = (Uint16)((Uint32)(len) / (Uint32)(sizeof(S_COMMUNICATION_RX_COMMAD) * 2)); //8BIT 단위의 Ethernet에서 수신된 BigEndian 자료수신 갯수
		...
		volatile S_COMMUNICATION_GSRAM_CPU2 *sp_GSRAM_CPU2, Uint16 *buf, Uint32 len)
{
	...
	ul_DataCountTotal = (Uint16)((Uint32)(len) / (Uint32)(sizeof(S_COMMUNICATION_RX_START) * 2)); //8BIT 단위의 Ethernet에서 수신된 BigEndian 자료수신 갯수
		...
		volatile S_COMMUNICATION_GSRAM_CPU2 *sp_GSRAM_CPU2, Uint16 *buf, Uint32 len)
{
	...
	ul_DataCountTotal = (Uint16)((Uint32)(len) / (Uint32)(sizeof(S_COMMUNICATION_RX_PAUSE) * 2)); //8BIT 단위의 Ethernet에서 수신된 BigEndian 자료수신 갯수
...
Uint16 COMMUNICATION_Ethernet_TempRead(volatile S_COMMUNICATION_RX_TEMP *chamber, Uint16 *buf, Uint32 len)
{
    ...
    for (ui_i = 0; ui_i < ul_DataCountTotal; ui_i++) {   //4개의 채널에서 해당 되는 Channel을 확인하고 해당 SETP DATA에 자료 저장
        ui_ChannelIndex = CLEAR;
        ul_DataCount = buf[1 + PointerCount];  //수신 DATA 총 갯수 정보 복사
        ui_Channel = buf[2 + PointerCount];  //장비 정보 복사
		...
		volatile S_COMMUNICATION_GSRAM_CPU1 *sp_GSRAM_CPU1, volatile S_COMMUNICATION_GSRAM_CPU2 *sp_GSRAM_CPU2)
{
	...
	{	//4개의 채널에서 해당 되는 Channel을 확인하고 해당 SETP DATA에 자료 저장
		...
		if(ui_Channel & (0x1 << ui_i))// && (sp_GSRAM_CPU1[ui_i]).CPU1_Step == (sp_GSRAM_CPU2[ui_i]).CPU2_Step)// (if))//데이타가 있는지 확인해야함. 일단 생략
		{
			...
			if((sp_GSRAM_CPU1[ui_i]).CPU1_Jump != (sp_GSRAM_CPU2[ui_i]).CPU2_Jump)// Jump 명령이 들어 왔을 경우.
			{
