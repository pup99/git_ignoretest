...
E_DSP_SCIA_FLAG DSP_SCIA_RxUpDate(volatile Uint16 *Data, volatile Uint16 *Buff, volatile Uint16 *Count, volatile Uint16 *Channel)
{
		...
		if(((*Count) >= DSP_SCIA_RUN_SYNC_MAX_BUFFER_LENGTH) &&
			(Buff[0] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[0] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)) &&
			(Buff[1] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[1] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)) &&
			(Buff[2] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[2] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)) &&
			(Buff[3] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[3] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)) &&
			(Buff[4] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[4] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)) &&
			(Buff[5] == (DSP_SCIA_RUN_SYNC_FLAG & 0x00FF) || Buff[5] == (DSP_SCIA_DATA_CALL_FLAG & 0x00FF)))
		{
			for(ui_i = 0, ui_j = 0; ui_i < (DSP_SCIA_RUN_SYNC_LENGTH); ui_i++)
			{
				Data[ui_i]  = ((Buff[ui_j++] << 8) & 0xFF00);
				Data[ui_i] |= ((Buff[ui_j++] << 0) & 0x00FF);
			}
			(*Count) = CLEAR;	//인터럽트 들어가려면 스킵해줘야 함.
			CRC16 = CRC16_Conversion((Uint16 *)Data, (DSP_SCIA_RUN_SYNC_LENGTH - DSP_SCIA_CRC16_LENGTH));
			if(CRC16 != Data[DSP_SCIA_RUN_SYNC_LENGTH - DSP_SCIA_CRC16_LENGTH])return(SCIA_ERROR_CRC16);
			(*Channel) = (Data[DSP_SCIA_START_FLAG_LENGTH] & 0x000F);
			//if((*Channel) > COMMUNICATION_CHANNEL)return(SCIA_ERROR_CHANNEL);

			if(Data[0] == DSP_SCIA_RUN_SYNC_FLAG)
			{
				(*Channel) = COMMUNICATION_CHANNEL_NUMBER;
				return(SCIA_SYNC);

			}
			else if(Data[0] == DSP_SCIA_DATA_CALL_FLAG)
			{
				return(SCIA_DATA);
			}
		}
